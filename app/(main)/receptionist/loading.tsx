...
function ListRowSkeleton() {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/5 px-4 py-3.5">
      <Skeleton className="h-7 w-7 rounded-lg bg-black/10" />
...
function RoomRowSkeleton() {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/5 px-4 py-3.5">
      <Skeleton className="h-6 w-9 rounded-lg bg-black/10" />
      <Skeleton className="h-4 flex-1 bg-black/10" />
      <Skeleton className="h-2 w-2 rounded-full bg-black/10" />
    </div>
  );
}
...
function BookingSection({
  ...
  return (
    <div className="relative space-y-4 rounded-3xl border border-black/10 p-4 pt-7">
      <div className="absolute -top-3 left-5 rounded-full border border-black/10 bg-[#0A8754] px-3 py-1">
          ...
          <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/20 bg-[#d4e2db] shadow-lg shadow-black/15">
            ...
            <div className="relative flex-1 space-y-6 overflow-hidden p-5">
              <BookingSection label="Date">
                <Skeleton className="h-12 w-full rounded-xl bg-black/10" />
              </BookingSection>

              <BookingSection label="Available time slots">
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <Skeleton key={i} className="h-10 rounded-xl bg-black/10" />
                  ))}
                </div>
              </BookingSection>
              ...
              <BookingSection label="Meeting details">
                ...
                <div className="flex justify-end pt-2">
                  <Skeleton className="h-11 w-40 rounded-xl bg-black/10" />
                </div>
              </BookingSection>
            </div>
          </div>
